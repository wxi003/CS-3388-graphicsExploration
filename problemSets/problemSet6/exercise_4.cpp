...
// New indices for the vertices
static const GLushort g_vertex_indices[] = {
    0, 1, 2 // Indices for the vertices
};

// specify the vertices and colors of a triangle using a single array
...
int main(int argc, char *argv[])
{
	...
	// Create a buffer and set the g_vertex_data as its data
	GLuint vertexBuffer;
	glGenBuffers(1, &vertexBuffer);
	glBindBuffer(GL_ARRAY_BUFFER, vertexBuffer);
	glBufferData(GL_ARRAY_BUFFER, sizeof(g_vertex_data), g_vertex_data, GL_STATIC_DRAW);

	// Create and bind IBO (Index Buffer Object)
	GLuint indexBuffer;
	...
	do
	{
		...
		glVertexAttribPointer(
			...
			(void *)0		   // pointer - the position data starts at the beginning of g_vertex_data
		);
		...
		glVertexAttribPointer(
			...
			GL_FALSE,					// normalized?
			6 * sizeof(float),			// stride
			(void *)(3 * sizeof(float)) // pointer - the color data starts after the first 3 floats of position data
		);

		// make sure to bind the vertex buffer for drawing
		glBindBuffer(GL_ARRAY_BUFFER, indexBuffer);
		glDrawElements(GL_TRIANGLES, 3, GL_UNSIGNED_SHORT, (void *)0); // 3 vertices needed to draw the triangle
